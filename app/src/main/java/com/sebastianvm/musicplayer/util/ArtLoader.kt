...
class ArtLoader {
    ...
    companion object {
        fun getAlbumArt(albumId: String, albumName: String): MediaArt {
            val uri =
                ContentUris.withAppendedId(
                    MediaStore.Audio.Albums.EXTERNAL_CONTENT_URI, albumId.toLong()
                )
            return MediaArt(
                uris = listOf(uri),
        ...
        fun getTrackArt(trackId: String, albumId: String, trackName: String): MediaArt {
            val tracksUri = MediaStore.Audio.Media.EXTERNAL_CONTENT_URI
            val albumsUri = MediaStore.Audio.Albums.EXTERNAL_CONTENT_URI
            val uris = listOf(
                ContentUris.withAppendedId(tracksUri, trackId.toLong()),
                ContentUris.withAppendedId(albumsUri, albumId.toLong())
            )
            return MediaArt(
                uris = uris,
