...
value class ResourceIcon(@DrawableRes val resId: Int) : IconState {
    @Composable
    override fun icon(): ImageVector = ImageVector.vectorResource(id = resId)
}

@JvmInline
value class VectorIcon(val imageVector: ImageVector) : IconState {
    @Composable
    override fun icon(): ImageVector = imageVector
}

@Composable
fun IconState.painter() = rememberVectorPainter(image = icon())
