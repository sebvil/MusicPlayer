...
class ArtistsMenuStateHolderTest : FreeSpec({
    ...
    fun TestScope.getSubject(media: HasArtists = MediaGroup.Album(ALBUM_ID)): ArtistsMenuStateHolder {
    ...
    "init sets state" - {
        "for Album" {
            ...
            artistRepositoryDep.albumsForArtists.value = artists.map {
                AlbumsForArtist(
                    albumId = ALBUM_ID,
                    artistId = it.id,
            ...
            testStateHolderState(subject) {
                ...
                awaitItemAs<Data<ArtistsMenuState>>().state shouldBe ArtistsMenuState(
        ...
        "for Track" {
            val artists = FixtureProvider.artistFixtures()
            artistRepositoryDep.artistTrackCrossRefs.value = artists.map {
                ArtistTrackCrossRef(
                    trackId = TRACK_ID,
                    artistId = it.id,
                    artistName = it.artistName,
